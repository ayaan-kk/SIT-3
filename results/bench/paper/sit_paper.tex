...
versus static partitioning with 0 catastrophic events.
A comprehensive 16-criterion scoreboard validates all layers: probe efficiency
ratio 0.31 (3$\times$ fewer probes than random), tomographic recall
1.0 with CI coverage 0.945, CVaR reduction 47.5\%
...
SIT-safe achieves:
\begin{itemize}
\item CVaR99: 47170.4 $\mu$s (ratio 1.23$\times$ vs partition's 38334.4 $\mu$s)
\item Effective Goodput: 8238.9 req/s (38.4\% higher than partition)
\item Success Rate: 0.9999
\item Catastrophes: 0
\item Oracle gap: 6182.4 $\mu$s
\end{itemize}

\subsection{Pareto Dominance Analysis}

The key insight is that SIT trades a modest CVaR99 increase (1.23$\times$
...
Static partitioning achieves the lowest CVaR99 (38334.4 $\mu$s) by
completely isolating workloads, but at the cost of wasted capacity:
partition's effective goodput is only 5951.5 req/s versus
SIT's 8238.9 req/s.

The oracle (with perfect ground-truth knowledge) achieves CVaR99 =
40988.0 $\mu$s, confirming that SIT's CVaR99 of 47170.4 $\mu$s
is within 6182.4 $\mu$s of the theoretical optimum.
...
\section{Comprehensive Evaluation Scoreboard}
...
\midrule
P1: Efficiency ratio & 0.31 & $\leq 0.40$ & PASS \\
P2: Diversity ratio & 0.29 & $\leq 0.70$ & PASS \\
...
Criterion & Value & Target & Status \\
\midrule
T1: Recall@$k$ & 1.0 & $\geq 0.97$ & PASS \\
T2: NDCG@$k$ & 0.9997 & $\geq 0.97$ & PASS \\
T3: Relative $L_2$ & 0.034 & $\leq 0.08$ & PASS \\
T4: CI coverage & 0.945 & $\geq 0.93$ & PASS \\
\bottomrule
\end{tabular}
...
S3: Detection recall & 1.0 & $\geq 0.98$ & PASS \\
\bottomrule
\end{tabular}
...
Criterion & Value & Target & Status \\
\midrule
L1: Goodput improvement & 38.4\% & $+20$--$40$\% & PASS \\
L2: Admitted load ratio & 1.39$\times$ & $\geq 1.30\times$ & PASS \\
L3: CVaR reduction & 47.5\% & $\geq 30$\% & PASS \\
L4: Pareto dominance & 100.0\% & $\geq 60$\% & PASS \\
\bottomrule
\end{tabular}

\noindent At high load, SIT achieves 47.5\% lower CVaR99
than the worst co-locating interference-unaware baseline (binpack-greedy),
while maintaining 38.4\% higher effective goodput than
...
SIT-safe achieves CVaR99 within 6182.4 $\mu$s of the oracle,
...
effective goodput versus static partitioning with 0 catastrophic
events across 91200 evaluation episodes. A comprehensive
