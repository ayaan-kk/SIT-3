...
and 4 load levels, SIT-safe reduces CVaR99 by -5.7\%
versus static partitioning while maintaining 0 catastrophic events.
...
that reduces tail latency by -5.7\% versus static partitioning
...
SIT-safe achieves:
\begin{itemize}
\item CVaR99: 34665.3 $\mu$s (-5.7\% improvement vs partition)
\item Goodput: 0.9999
\item Catastrophes: 0
\item Oracle gap: 1142.6 $\mu$s
\end{itemize}
...
are safe, achieving -5.7\% lower CVaR99 while
maintaining comparable safety guarantees.

\paragraph{The oracle gap.}
SIT-safe achieves CVaR99 within 1142.6 $\mu$s of the oracle,
...
-5.7\% versus static partitioning while
preventing catastrophic placements across 91200
